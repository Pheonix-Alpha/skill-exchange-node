    ...
    <section className="bg-[#0D0D14] py-24 px-6 border-t border-white/5">
      <div className="max-w-[1200px] mx-auto">
        <div className="text-center mb-16">
          <p className="text-[#4F8EF7] text-xs font-bold uppercase tracking-[0.2em] mb-3">Simple process</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {steps.map((s, i) => {
            const Icon = s.icon;
            return (
              <div key={i} className={`relative p-6 rounded-2xl border ${s.border} bg-white/[0.02] hover:bg-white/[0.04] transition-all group`}>
                ...
                <div
                  className="absolute top-5 right-5 text-4xl font-black text-white/[0.04] group-hover:text-white/[0.07] transition-colors"
        ...
        <div className="mt-16 rounded-2xl border border-[#4F8EF7]/20 bg-[#4F8EF7]/5 p-8 md:p-10 flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1">
            ...
            <p className="text-white/40 text-sm">Get notified of new skills, features and community highlights.</p>
          </div>
